...
`--auto-approve` is an explicit opt-in for **this immutable job's transfer and literal task execution**, not a general `--yes`, secret-validation bypass, or unsupported harness permission flag. Use `--prepare` followed by the sensitive `inspect` command to review the full inventory, digest, paths, effective profile, requirements, exclusions and exact task/context before authorized approval; `autoapproval.json` durably binds it to that job/digest/destination. Without it, a TTY and exact digest approval are required; non-TTY invocation refuses before any dispatch. The controlled native tools already run with the destination user's OS permissions. Use an independently isolated account/container for unattended untrusted work.
...
`--name` is an optional safe ASCII handle (`letters/digits/_.-`, first character alphanumeric). UUIDs remain authoritative; duplicate names are allowed but name lookup rejects ambiguity. `ls` shows name/UUID/host/paths/ownership/execution without sensitive instruction text. Workspaces go under `<codeRoot>/<name>-<uuid>` (or `<codeRoot>/<uuid>`); without `codeRoot`, the existing state-root run layout is retained. Git object storage and native state stay in Bauble state. Each run has an isolated tmux server; ordinary `tmux ls` does **not** list them. Use `open <name-or-id> [--here]`, which verifies and attaches the same process, never starts another.
...
Remote commands now share one versioned streaming RPC connection per host (terminal attachment remains separate). Both ends must pass the current-release/capability handshake; there is no automatic downgrade, reconnect, or replay. Durable intent IDs go to stderr even with `--json --quiet`. See `bauble docs configuration` for transport bounds and `bauble docs recovery` for capture intents, route-only return observation, and loss handling. Offline measured helper counts and timing are in `bauble docs automation`; live SSH still requires explicit host authorization and validation.
