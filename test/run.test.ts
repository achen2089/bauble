...
test('run CLI: non-TTY refuses default approval; autoapprove starts native tmux, codeRoot/name routing, host defaults, literal input', { timeout: 90000 }, async t => {
  ...
  const blocked = call('run', f.cwd, '--prompt', 'sensitive-literal-marker', '--json', '--quiet'); assert.equal(blocked.status, 3); const blockedEnvelope = JSON.parse(blocked.stdout); assert.equal(blockedEnvelope.error.code, 'APPROVAL_REQUIRED'); assert.ok(blockedEnvelope.data.transferId); assert.ok(!blocked.stdout.includes('sensitive-literal-marker')); assert.equal(blocked.stderr, '');
