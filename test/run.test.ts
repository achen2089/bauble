...
for (const git of [false, true]) test(`run: fresh native ${git ? 'dirty Git' : 'plain folder'} tool task, no seed, lost ACK no replay, messages and separate result pull`, async t => {
  ...
  await assert.rejects(dispatchTask(f.source, id, lost, f.remoteRoot), /lost launch ACK/);
...
test('run CLI: non-TTY refuses default approval; autoapprove starts native tmux, codeRoot/name routing, host defaults, literal input', { timeout: 90000 }, async t => {
  ...
  const env = { ...process.env, PATH: `${bin}:${process.env.PATH}`, BAUBLE_CONFIG: configFile, BAUBLE_STATE: f.source.root, BAUBLE_TEST_MODE: '1', BAUBLE_TEST_REMOTE_CONFIG: remoteConfig, BAUBLE_TEST_REMOTE_ROOT: f.remoteRoot, BAUBLE_TEST_PROFILE_DIGEST: f.digest, PI_OFFLINE: '1' };
  const tokens: string[] = [];
  ...
  const call = (...args: string[]) => spawnSync(process.execPath, [cli, ...args], { env, encoding: 'utf8', timeout: 45000, maxBuffer: 4 * 1024 * 1024 });
  const remoteBefore = readFileSync(remoteConfig); const added = call('host', 'add', 'second', '--default', '--code-root', codeRoot); assert.equal(added.status, 0, added.stderr); assert.equal(JSON.parse(readFileSync(remoteConfig, 'utf8')).remoteRoot, f.config.remoteRoot); const backup = readdirSync(f.root).find(p => p.startsWith('remote.json.') && p.endsWith('.bak'))!; assert.deepEqual(readFileSync(join(f.root, backup)), remoteBefore); assert.ok(call('host', 'list').stdout.includes('second')); assert.equal(call('host', 'default', 'remote').status, 0);
  const bootstrapFile = join(f.root, 'bootstrap.json'); const bootstrap = spawnSync(process.execPath, [cli, 'host', 'add', 'remote', '--profile', f.profile.path], { env: { ...env, BAUBLE_CONFIG: bootstrapFile }, encoding: 'utf8', timeout: 10000 }); assert.equal(bootstrap.status, 0, bootstrap.stderr); assert.ok(bootstrap.stdout.includes('No ambient')); assert.equal(JSON.parse(readFileSync(bootstrapFile, 'utf8')).profile, f.profile.path);
  const refused = call('run', f.cwd, '--prompt', 'literal /bauble'); assert.notEqual(refused.status, 0); assert.match(refused.stderr, /Interactive approval required/); assert.equal(f.remote.list().length, 0); assert.equal(existsSync(join(f.source.root, 'sessions')), false);
  ...
  assert.ok(existsSync(join(f.source.transfer(job.transferId), 'autoapproval.json'))); assert.ok(call('ls').stdout.includes(`socket=${job.receipt!.socket}`));
...
test('run: pre-dispatch changes and lost-before-activation authority never launch or replay', async t => {
  ...
  await assert.rejects(dispatch, /crashed before/);
