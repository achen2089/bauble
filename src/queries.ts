...
export function resolveTransfer(input: string, stores = localStores()) {
  const matches = Id.safeParse(input).success ? stores.filter(s => existsSync(join(s.transfer(input), 'manifest.json'))).map(store => ({ store, id: input })) : stores.flatMap(store => store.list().filter(r => r.manifest.name === input).map(r => ({ store, id: r.transferId })));
...
export function nextFor(store: Store, id: string) { const { digest } = store.manifest(id); const status = store.status(id); if (status.phase === 'cancelled') return [action('Read retained cancellation snapshot', 'read', 'inspect', id)]; if (!existsSync(join(store.transfer(id), 'approval.json'))) return approvalActions(id, digest); return [action('Observe cached state', 'read', 'status', id), action('Reconcile this exact ID if authorized', 'mutate', 'recover', id)]; }
