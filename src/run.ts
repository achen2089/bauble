...
export interface RunOptions { prepare?: boolean; cwd?: string; task?: string; prompt?: string; context?: string[]; host?: string; name?: string; profile?: string; autoApprove?: boolean; sensitive?: string[]; history?: string[] }
...
export async function captureTask(options: RunOptions, store: Store, target: { alias: string; root: string; codeRoot?: string; profile: string; profileDigest: string }) {
  ...
  const manifest = Manifest.parse({ protocol: 1, transferId, lineageId, parentTransfer: null, generation: 1, created: new Date().toISOString(), agent: 'pi', piVersion: PI_VERSION, destination: target.alias, source: { repository: root, cwd }, target: { repository: targetRepo, cwd: join(targetRepo, relative(root, cwd)) }, instruction, workspace, native, inputs, ...(name ? { name } : {}), ...(target.codeRoot ? { codeRoot: resolve(target.codeRoot) } : {}), blobs: refs.map(hash => ({ hash, size: store.blobs.get(hash).length })) });
...
export async function dispatchTask(store: Store, id: string, rpc: Rpc, root: string, context: OperationContext = {}) {
...
export async function runTask(options: RunOptions, store = new Store(), connect: (alias: string) => Rpc = ssh, context: OperationContext = {}) {
  invariant(!options.prepare || !options.autoApprove, '--prepare conflicts with auto-approval');
  const config = loadConfig(); const alias = selectHost(config, options.host); const host = config.hosts[alias]!;
  context.progress?.('capture');
  const { manifest } = await captureTask(options, store, { alias, ...host, profile: config.profile }); const id = manifest.transferId;
  const snapshot = prepared(store, id, false); context.onDurable?.(snapshot);
  if (options.prepare) return snapshot;
  await approve(store, id, undefined, undefined, options.autoApprove, context);
  return dispatchTask(store, id, connect(alias), host.root, context);
}
